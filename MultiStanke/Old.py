...
class Game:
    def __init__(self, V_INIT, R_INIT, V_PAS, R_Pas, P, PV):
        ...
        self.snakes.append(
            Snake(green, [400, random.randrange(1, (window_y // 10)) * 10], V_INIT, P, PV, R_INIT, False))
        ...
        if self.snakes:
            ...
            try:
                ...
                if (self.N_GENCounter == N_GEN):
                    ...
                    self.fruits.append(
                        Food(NM + random.randint(-1, 2), N_EXP + random.randint(-2, 2), fruit_pos,
                             random.random() > 0.8))
                    for fruit in self.fruits:
                        if fruit.poisoned:
                ...
                ####A *
                sightMatrix = get_submatrix(self.map, (
                int(self.snakes[index].body[0][0] / 10), int(self.snakes[index].body[0][1] / 10)),
                                            self.snakes[index].RInit)
...
'''def gameTick(gameInstance, snake, action):
    global score, map, window_x, window_y, N_GENCounter, snakes
    if snakes:
        flag = False
        if (len(snake.body) < 3):
            snakes.remove(snake)
            flag = True
        snake.frameCounter = snake.frameCounter + 1
        if (snake.frameCounter == snake.speed):
            snake.frameCounter = 0
            # if (len(snake.body) >= 3):
            if (snake.move(action)):
                # print("a")
                flag = True
                snakes.remove(snake)

        snake.pCounter = snake.pCounter + 1
        snake.pvCounter = snake.pvCounter + 1

        if (snake.pCounter == snake.P):
            x = snake.body.pop()
            map[int(x[0] / 10)][int(x[1] / 10)] = 0
            snake.pCounter = 0
        if (snake.pvCounter == snake.PV):
            snake.speed = snake.speed + V_PAS  # inversely proportional
            snake.RInit = snake.RInit - R_Pas
            snake.pvCounter = 0

        for fruit in fruits:
            fruit.expirationCounter = fruit.expirationCounter + 1
            if snake.body[0] == fruit.fruit_position:
                score += 10
                snake.fitness += 100
                # fruit_spawn = False
                if (fruit.poisoned):
                    if (len(snake.body) - fruit.NM < 3):
                        snakes.remove(snake)
                    else:
                        snake.grow_snake(fruit.NM * -1)
                else:
                    snake.grow_snake(fruit.NM)
                fruits.remove(fruit)

            if (fruit.expirationCounter == fruit.NExp):
                fruits.remove(fruit)
        N_GENCounter = N_GENCounter + 1
        # print(N_GENCounter)
        # print(N_GEN)
        if (N_GENCounter == N_GEN):
            N_GENCounter = 0
            fruit_pos = [random.randrange(1, (window_x // 10)) * 10, random.randrange(1, (window_y // 10)) * 10]
            fruits.append(
                Food(NM + random.randint(-1, 2), N_EXP + random.randint(-2, 2), fruit_pos, random.random() > 0.8))
            if fruit.poisoned:
                map[int(fruit_pos[0] / 10)][int(fruit_pos[1] / 10)] = 4
            else:
                map[int(fruit_pos[0] / 10)][int(fruit_pos[1] / 10)] = 3

        ####A *
        sightMatrix = get_submatrix(map, (int(snake.body[0][0] / 10), int(snake.body[0][1] / 10)), snake.RInit)
        sightMatrixRows = len(sightMatrix)
        sightMatrixCols = len(sightMatrix[0])
        ...
        distance = 9999
        poisonedDistance = 9999
        start = (snake.RInit, snake.RInit)
        for r in range(sightMatrixRows):
            for c in range(sightMatrixCols):
                if (sightMatrix[r][c] == 3 and abs(r - R_INIT) + abs(c - R_INIT) < distance):
                    goal = (r, c)
                    distance = abs(r - R_INIT) + abs(c - R_INIT)
                if (sightMatrix[r][c] == 4 and abs(r - R_INIT) + abs(c - R_INIT) < poisonedDistance):
                    poisonedGoal = (r, c)
                    poisonedDistance = abs(r - R_INIT) + abs(c - R_INIT)
        if (goal):
            snake.foodPath = Astar.a_star_search(sightMatrix, start, goal)
        else:
            snake.foodPath = []

        if (poisonedGoal):
            snake.poisonedFoodPath = Astar.a_star_search(sightMatrix, start, poisonedGoal)
        else:
            snake.poisonedFoodPath = []
        snake.getOutput(sightMatrix)
        # print(len(snake.output))
        # print(snake.output)
...
'''
'''# Main Function
def play(score, snakes, fruits, map, window_x, window_y, game_window, fps, N_GENCounter, action):
    print("test")
    while True:
        if snakes:
            count = 0
            ...
            # print (f"actual{len(snakes[0].body)}")
            for event in pygame.event.get():
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_ESCAPE:
                        pygame.quit()
            ...
            for snake in snakes:
                ...
                if (goal):
                    snake.foodPath = Astar.a_star_search(sightMatrix, start, goal)
                    # print(snake.foodPath[1])
                    #if snake.foodPath:
                    #    print("Path found:")
                    #    for step in snake.foodPath:
                    #        print(step)
                    #else:
                    #    print("No path found")
                else:
